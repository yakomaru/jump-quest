export const TILE_SIZE = 16;

export const GRAVITY        = 500;
export const WALK_SPEED     = 120;
export const AIR_CONTROL    = 0.6;
export const JUMP_VELOCITY  = -320;
export const JUMP_HOLD_GRAV = 180;
export const JUMP_HOLD_MAX  = 250;
export const DRAG_GROUND    = 800;
export const DRAG_AIR       = 200;
export const COYOTE_TIME    = 80;
export const JUMP_BUFFER    = 100;

export const KNOCKBACK_X    = 180;
export const KNOCKBACK_Y    = -220;
export const HURT_DURATION  = 600;

export const ENEMY_SPEED    = 50;
export const PATROL_DIST    = 80;

export const MAX_LIVES      = 3;
